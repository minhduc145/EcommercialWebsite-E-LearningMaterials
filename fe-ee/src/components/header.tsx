...
export default function Header(props: IHeaderProps) {
	...
	useEffect(() => {
		const newMessageSocket = new SockJS('http://localhost:8080/ws/notification');
		const newMessageClient = new Client({
			webSocketFactory: () => newMessageSocket,
			reconnectDelay: 5000,
			onConnect: () => {
				console.log('✅ Connected to WebSocket');
				newMessageClient.subscribe(`/topic/receive/${JSON.parse(String(localStorage.getItem('currentUser')))?.id}`
					, (message: IMessage) => {
